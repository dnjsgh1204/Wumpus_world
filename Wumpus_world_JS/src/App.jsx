...
const App = () => {
  ...
  useEffect(() => {
    ...
    // Check if the initial agent position contains a stench
    if (stenchPositions.some(pos => pos.x === agentPosition.x && pos.y === agentPosition.y)) {
      shootArrow(agentPosition);
    }
    ...
    if (lowestScorePosition) {
      ...
      setTimeout(() => {
        ...
        if (lowestScorePosition.x === wumpusPosition.x && lowestScorePosition.y === wumpusPosition.y) {
          ...
          // Wumpus가 죽으면 해당 위치의 점수를 +5점 증가
          const newScoreBoard = scoreBoard.map(row => [...row]);
          newScoreBoard[lowestScorePosition.x][lowestScorePosition.y] += 5;
